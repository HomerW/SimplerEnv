...
import simpler_env
from simpler_env import ENVIRONMENTS, GOOGLE_ENVIRONMENTS, BRIDGE_ENVIRONMENTS
...
if args.policy == "rt1":
    ...
    if "cross_embodiment" in args.ckpt_path or "baseline" in args.ckpt_path:
...
mean_successes = []
# for task in BRIDGE_ENVIRONMENTS:
for task in GOOGLE_ENVIRONMENTS:
# for task in ["widowx_spoon_on_towel", "widowx_carrot_on_plate", "widowx_stack_cube", "widowx_put_eggplant_in_basket"]:
# for task in ["google_robot_pick_coke_can"]:

    logging_dir = os.path.join(args.logging_root, task, args.policy, os.path.basename(args.ckpt_path), str(args.step))
    ...
    mean_successes.append(mean_success)
...
print(f"Mean success: {mean_successes} = {np.mean(mean_successes)}")
